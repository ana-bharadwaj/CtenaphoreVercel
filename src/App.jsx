...
export default function ImageCategorizer() {
  const options = ["202502-1", "202502-2", "202502-3", "202502-4"];

  // mainImage: { blobPath, displayUrl } or null
  const [mainImage, setMainImage] = useState(null);

  // choiceImages: {
  //   "202502-1": { blobPath, displayUrl },
  //   "202502-2": { blobPath, displayUrl },
  //   ...
  // }
  ...
  // get a fresh random image set from Flask
  const fetchImages = useCallback(() => {
    setLoading(true);
    fetch("http://127.0.0.1:5000/api/image-set")
      .then((res) => res.json())
      .then((data) => {
        // data.mainImage = { blobPath, displayUrl }
        // data.choices  = { "202502-1": { blobPath, displayUrl }, ... }
  ...
  const submitCard = (idx, choice) => {
    ...
    fetch("http://127.0.0.1:5000/api/submit-label", {
      ...
      body: JSON.stringify({
        username: user.email,
        mainImageBlob: mainImage.blobPath, // <-- store clean path in Firestore
        choice: choice,
      }),
    })
      .then((res) => res.json())
                ...
                <div className="rightPane">
                  ...
                  <div className="choicesCol">
                    {options.map((opt) => {
                      ...
                      // this is { blobPath, displayUrl } or undefined
